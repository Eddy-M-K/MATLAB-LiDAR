function [Export] = Perpendicular_Rotated_Precise_Moving_Golf_Ball_Intersection(Precise_Azimuth, sphere_moving_x, sphere_moving_y, sphere_moving_z, r, Precise_Azimuth_Resolution, Radius_Start, Radius_End, LiDAR_x, LiDAR_y, LiDAR_z, Export)
    fprintf("Rotated Precise Moving Golf Ball Intersection (Perpendicular to Golf Ball Flight):\n");
    position = 1;
    while position <= length(sphere_moving_x)
        for Vertical_Angle = 15:-2:-15
            ...
            P_End = [Radius_End*sind(Vertical_Angle) + LiDAR_x, ...
                    Radius_End*cosd(Precise_Azimuth)*cosd(Vertical_Angle) + LiDAR_y, ...
    ...
    title('Perpendicular Rotated Precise Moving Golf Ball Intersection')
    xlabel('X')
    ylabel('Y')
    zlabel('Z')
    daspect([1 1 1])
